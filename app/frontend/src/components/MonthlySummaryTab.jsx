...
function ScopePanel({ title, tone, data }) {
  ...
  return (
    <div className={`bg-card border border-border rounded-md ${accent}`}>
      <div className="p-6 pb-4">
        ...
        <div className="grid grid-cols-3 gap-4 mt-3">
          <div>
            ...
            <div className="font-mono text-lg font-semibold text-business mt-1">{formatINR(income)}</div>
          </div>
          <div>
            <div className="text-[11px] text-muted-foreground uppercase tracking-widest">Expense</div>
            <div className="font-mono text-lg font-semibold text-foreground mt-1">{formatINR(expense)}</div>
          </div>
          <div>
            <div className="text-[11px] text-muted-foreground uppercase tracking-widest">Net</div>
            <div className={`font-mono text-lg font-semibold mt-1 ${net >= 0 ? "text-business" : "text-destructive"}`}>
            ...
            <TableBody>
              {byCat.map((c) => (
                <TableRow key={c.name} className="hover:bg-muted/40">
                  ...
                  <TableCell className={`text-right font-mono text-xs ${c.net >= 0 ? "text-business" : "text-destructive"}`}>
